...
const Selection = ({ selectedDifficulty, handleDifficultyChange }: Props) => {
  ...
  const selectedStatus = invoiceStatuses.find(
    (status) => status.value === selectedDifficulty
  );
  ...
  return (
    <div className={[style.difficultyContainer,"font-Inter"].join(" ")}>
      <p className={[style.heading,"font-Inter"].join(" ")}>Select Difficulty Level</p>
          ...
          >
            ...
            <div className={style.dropdownMenu} role="listbox">
              {invoiceStatuses.map((status) => (
                <button key={status.value} className={selectedStatus?.id === status.id ? [ style.dropdownItem,style[customColorClass[selectedStatus.value]],style.selectedOption].join(" ") : style.dropdownItem } onClick={() => handleSelect(status.value)} role="option" aria-selected={selectedStatus?.id === status.id}>
