...
const App = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<Category | null>(null)  //null {id:9,name:"General knowledge"}
  const [selectedType,setSelectedType] = useState<Type | null>(null) //null  {id:1,name:"MCQ",fullName:"Multiple Choice Questions"}
  const [selectedDifficulty,setSelectedDifficulty] = useState<string | null>("any");
  ...
  function handleActiveStep(step : number,op : string){
    ...
    if(op === "up" && activeStep === 2){
      selectedDifficulty && selectedType && isValid && setActiveStep(step); setShouldShowQuestion(true);
    }
  ...
  return (
    <>
      ...
      <Homepage></Homepage>
      {(!shouldShowQuestion &&<Card questionsCount={count} isValid={isValid} activeStep={activeStep} handleActiveStep={handleActiveStep} selectedCategory={selectedCategory} handleTypeChange={handleTypeChange} selectedType={selectedType} handleClick={handleCategoryChange} selectedDifficulty={selectedDifficulty} handleDifficultyChange={handleDifficultyChange} formValidation={formValidation} handleQuestionCountChange={handleQuestionCountChange} ></Card>)}
      {shouldShowQuestion && <MCQWorkspace selectedType={selectedType as Type} category={selectedCategory as Category} count={count} difficulty={selectedDifficulty as string} handleRetake={() => console.log("RETAKE")} handleReturnFromType={handleReturnFromType}></MCQWorkspace>
}
